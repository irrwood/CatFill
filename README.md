...
9. **公司信息入口**：在招聘页面打开 popup 时，CatFill 会在本机根据招聘页的结构化数据、页面品牌和常见 ATS 地址识别招聘公司，并提供 LinkedIn 公司搜索与 Glassdoor 评价搜索入口。只有点击链接时才会打开对应网站；识别过程不上传页面内容。可以在「设置 → 通用设置 → 识别招聘公司」中随时关闭。
