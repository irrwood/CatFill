...
9. **公司信息入口**：在招聘页面打开 popup 时，CatFill 会在本机根据招聘页的结构化数据、页面品牌和常见 ATS 地址识别招聘公司，并提供 LinkedIn 与 Glassdoor 入口。点击 Glassdoor 后，CatFill 会按需申请 Glassdoor 可选权限，用公司名换取公开 employer ID；名称完全一致时直达 Overview 页面，有歧义时显示候选公司，失败时才提供限定搜索。可以在「设置 → 通用设置 → 识别招聘公司」中随时关闭。
